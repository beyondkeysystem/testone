 ...
 */
 	...
 	audio_addEvent : function(){
 		...
 		function audio_txt(txt,val,time_txt){
 			if(val) txt.text('Turn-on');
 			else txt.text('Turn-off');
 ...
 */
	//插件启动函数
 	plugin : function(){
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams				: ["<img src='/assets/mobile/images/audio_widget_01@2x.png' />","<img src='/assets/mobile/images/audio_widget_01%402x.png' />"], 
			steamHeight			: 100,
			steamWidth			: 44 
		});
 ...
 */
	...
	init : function(){
		// 样式，标签的渲染
		// 对象操作事件处理
	//car2._page.last().before('<div class="m-page m-bigTxt f-hide" data-page-type="bigTxt" data-statics="info_list"><div class="page-con j-txtWrap lazy-img" data-src="/assets/mobile/images/1.jpg"></div></div>');
...
function backToIndex(){//看广告回退时跳转到计数页
    var path=location.pathname,
        id=path.split('http://www.5.cn/')[3],//当前杂志的唯一id
