...
$auth_data = $this->Auth->user();
	...
	function postadd(){
		...
		if(isset($this->params['pass'][0])){
			//echo "-----<br>----";
			$ad_id=base64_decode($this->params['pass'][0]);
			$jobAdData = $this->JobAd->find('first', array('conditions' => array('ad_id' => $ad_id)));
			//$query = $this->JobAd->save($data['JobAd']);
			$this->set('jobAdData',$jobAdData);
			//echo $this->params['pass'][0]; pr($jobAdData); exit;
			$level = $jobAdData['JobAd']['level'];$other_postal_city= '';
			$industry = $jobAdData['JobAd']['industry'];
			$ad_country = $jobAdData['JobAd']['ad_country'];
			$equity_status = $jobAdData['JobAd']['filter_equity_status'];
			$citizenship = $jobAdData['JobAd']['filter_citizenship'];
			$min_sec_qualification = $jobAdData['JobAd']['min_sec_qualification'];
			$exp_required = $jobAdData['JobAd']['exp_required'];
			$off_package4 = $jobAdData['JobAd']['off_package4'];
			$off_package5 = $jobAdData['JobAd']['off_package5'];
			$off_package6 = $jobAdData['JobAd']['off_package6'];
			$language_filter = $jobAdData['JobAd']['filter_language_filter'];
			$ter_qualification = $jobAdData['JobAd']['min_ter_qualification'];
			$level_filter = $jobAdData['JobAd']['filter_level_filter'];
			$industry_filter = $jobAdData['JobAd']['filter_industry_filter'];
			$profession_filter = '';
		}
		else{
			$level = '';
			...
			$industry = '';
			$ad_country = '';
			$citizenship = "";$equity_status = '';$min_sec_qualification = '';$exp_required = '';$off_package4 = '';$off_package5 = '';
			$off_package6 = '';$language_filter = '';$ter_qualification = '';$level_filter = ''; $profession_filter = '';$industry_filter = '';
		}
...
$auth_data = $this->Auth->user();
		...
		if(isset($_POST["off_package_benefits"])){
			$array2["off_package_benefits"] = serialize($_POST["off_package_benefits"]);
		}
		...
		if(isset($_POST['jobAddRefNo']))
		{
			...
			$array2['filter_gender']=$_POST['gender'];
			$data['JobAd'] = $array2; 
			if($_POST['ad_id']=='' && !is_numeric($_POST['ad_id'])){
				$this->JobAd->create();
			}
			else{
				$array2['ad_id']=$_POST['ad_id'];
			}			
			$query = $this->JobAd->save($data['JobAd']);
...
$auth_data = $this->Auth->user();
	...
	function deletejobad() {
		$this->layout = "ajax";
		$this->autoRender = false;
		if ($this->request->is('ajax')) {
			$this->JobAd->id=$_POST['ad_id'];
			if($this->JobAd->delete()){
				echo "Success";
			}
			else{
				echo $_POST['ad_id'];
...
require_once(dirname(dirname(__file__)) . '/Vendor/im/global.php');
	...
	function profile_edit()
	{
		...
		if ($this->request->is('ajax')) {
			...
			{
				$getdata = $this->EmployerProfile->find('first', array(
			        'conditions' => array('EmployerProfile.emp_id' => $_REQUEST['id'] )));
				if($getdata){
					$data['EmployerProfile']['id'] = $getdata['EmployerProfile']['id'];
					$data['EmployerProfile']['company_contact'] = $_REQUEST['pval'];
				}else{
					$data['EmployerProfile']['emp_id'] = $_REQUEST['id'];
					$data['EmployerProfile']['company_contact'] = $_REQUEST['pval'];
					$this->EmployerProfile->create();
				}

				$query = $this->EmployerProfile->save($data['EmployerProfile']);
				$this->Session->setFlash(__('Profile data updated.'));
			}
			if(isset($_REQUEST['type']) AND $_REQUEST['type'] == 'name')
			{
				...
				if($getdata){
					$data['EmployerProfile']['id'] = $getdata['EmployerProfile']['id'];
					$data['EmployerProfile']['emp_company_name'] = $_REQUEST['pval'];
				}else{
					$data['EmployerProfile']['emp_id'] = $_REQUEST['id'];
					$data['EmployerProfile']['emp_company_name'] = $_REQUEST['pval'];
					$this->EmployerProfile->create();
			...
			if(isset($_REQUEST['type']) AND $_REQUEST['type'] == 'summary')
			{
				...
				if($getdata){
					$data['EmployerProfile']['id'] = $getdata['EmployerProfile']['id'];
					$data['EmployerProfile']['business_summary'] = $_REQUEST['sval'];
				}else{
					$data['EmployerProfile']['emp_id'] = $_REQUEST['id'];
					$data['EmployerProfile']['business_summary'] = $_REQUEST['sval'];
					$this->EmployerProfile->create();
			...
			if(isset($_REQUEST['type']) AND $_REQUEST['type'] == 'proimage')
			{
				...
				$prefix = time() . '_';
			if(isset($_FILES['0']['name']) AND $_FILES['0']['name'] != '')
			{
				
				$filename = $prefix.basename($_FILES['0']['name']);
				$file_ext = strtolower(substr($filename, strrpos($filename, ".") + 1));
				$logo_filename = String::uuid().".".$file_ext;
				$height ='202';
				$width = '189';
				$comp_logo = $this->JobportalImage->uploadImage($_FILES['0'], $_SERVER['DOCUMENT_ROOT'].$this->webroot.'/app/webroot/uploads/', $logo_filename, $height , $width);
			}
				...
				if($getdata){
					$data['EmployerProfile']['id'] = $getdata['EmployerProfile']['id'];
					$data['EmployerProfile']['company_logo'] = $logo_filename;
				}else{
					$data['EmployerProfile']['emp_id'] = $_REQUEST['id'];
					$data['EmployerProfile']['company_logo'] = $logo_filename;
					$this->EmployerProfile->create();
				...
				echo '<img src="'.$this->webroot.'uploads/'.$logo_filename.'" alt="" />';
			}
			if(isset($_REQUEST['type']) AND $_REQUEST['type'] == 'banner')
			{
					$prefix = time() . '_';
			if(isset($_FILES['0']['name']) AND $_FILES['0']['name'] != '')
			{
				...
				$height ='70';
				$width = '720';
				$comp_banner = $this->JobportalImage->uploadImage($_FILES['0'], $_SERVER['DOCUMENT_ROOT'].$this->webroot.'/app/webroot/uploads/', $company_banner, $height , $width);
			}
				...
				if($getdata){
					$data['EmployerProfile']['id'] = $getdata['EmployerProfile']['id'];
					$data['EmployerProfile']['company_banner'] = $company_banner;
				}else{
					$data['EmployerProfile']['emp_id'] = $_REQUEST['id'];
					$data['EmployerProfile']['company_banner'] = $company_banner;
					$this->EmployerProfile->create();
				...
				$query = $this->EmployerProfile->save($data['EmployerProfile']);
				$this->Session->setFlash(__('Profile data updated.'));
				echo '<img src="'.$this->webroot.'uploads/'.$company_banner.'" alt="company_banner" />';
			}
