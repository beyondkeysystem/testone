<?php echo $this->element('left_employer_profile_sidebar');  ?>
	...
	<?php	if(isset($pro_data['company_banner']))	{ ?>
		<div class="profile_name">
		    <a href="javascript:void(0);" id="banner" onClick="pop('popDiv1')">
			<img src="<?php echo $this->webroot.'uploads/'.$pro_data['company_banner']; ?>" alt="company_banner" />
		    </a>
		<!--<h1 class="lh60">Company Banner</h1>-->
		</div>
		<?php } ?>
		<div id="popDiv1" class="ontop1">
		<div id="popup1">
		    <div style="text-align: right;" ><a href="javascript:void(0);"  onClick="hide('popDiv1')">Close</a></div>
		    <form action="" method="post" id="bannerfrm" name="bannerfrm" enctype="multipart/form-data">
			<div>
...
<div id="loader1" style="display:none;">
			    ...
			    <div>
				<input class="blue_btn pull-right" type="submit" name="submit_banner" value="Update">
			    </div>
			</div>
		    </form>
	        </div>
	    ...
	    <?php } ?>
            ...
            <div class="content_tab ct_employer" id="horizontalTab">
                <ul>
                    ...
                    <li><a href="javascript:void(0);" onclick="javascript:window.location.href='<?=$this->webroot?>chat/room/'"><i class="fa fa-users"></i>Chat Room</a></li>
...
//pr($job_info);
			 ...
			 echo $this->Html->link(__('Click To Apply'), array('controller' => 'users', 'action' => 'login'),
				...
				?>
				<div class="login-box">
				    <h3 class="form-group" style="margin-left: 15px;">
					<?php echo $this->Html->link(__('See All Jobs'), array('controller' => 'employer', 'action' => 'alljobs/'.$emp_user), array('class'=>'btn btn-primary')); ?>
				    </h3>
				</div>
				<?php				
		}		
