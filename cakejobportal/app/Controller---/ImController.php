...
class ImController extends AppController {
	...
	$str = '<ul class="instant_msg">';
		...
		for ($i = 0; $i <= count($user_info); $i++) {
			...
			if($user_data['User']['status']=='active'){
			    ...
			    $str .= '<li>
				<a href="javascript:void(0);" onclick="getpopup(\''. base64_encode($r->trans_filmmakers_chat->from_id).'\','.$_REQUEST['video_id'].')" class="inst_msg_img"><img src="' . $profile_image . '" alt="" /></a>
				<a href="javascript:void(0);" onclick="getpopup(\''. base64_encode($r->trans_filmmakers_chat->from_id).'\','.$_REQUEST['video_id'].')" class="inst_msg_txt">' . $user_data['User']['name']. '<span class="status"></span></a>
				<div class="clearfix"></div>
			    </li>';
			}
			else{
			    ...
			    $str .= '<li>
				<a href="javascript:void(0);" onclick="getpopup(\''. base64_encode($r->trans_filmmakers_chat->from_id).'\','.$_REQUEST['video_id'].')" class="inst_msg_img"><img src="' . $profile_image . '" alt="" /></a>
				<a href="javascript:void(0);" onclick="getpopup(\''. base64_encode($r->trans_filmmakers_chat->from_id).'\','.$_REQUEST['video_id'].')" class="inst_msg_txt">' . $user_data['User']['name']. '<span class="off"></span></a>
				<div class="clearfix"></div>
			    </li>';
			}
    ...
    function my_chat_room() {
	    ...
	    $_SESSION['valpopup']=0;
	    $this->set('valpopuppx', $_SESSION['valpopup']);
	}
	else{
	    $valpopup = $_SESSION['valpopup'];
	    $_SESSION['valpopup'] = $valpopup + 225;
	    $this->set('valpopuppx', $_SESSION['valpopup']);
	}
