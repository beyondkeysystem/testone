...
//print_r($cur_user);
	if (!empty($cur_user['password']))
	{
		...
		// Otherwise we should have a normal sha1 password
			...
			$authorized = ($cur_user['password'] == $form_password_hash)?true:false;
		/*echo sha1($cur_user['salt']);
		echo "<br>";
		echo sha1($form_password);
		echo "<br>";
		echo sha1($cur_user['salt'].sha1($form_password));
		echo "<br>";
		echo $cur_user['password']; exit;*/
	}

	if (!$authorized)
		message($lang_login['Wrong user/pass'].' <a href="login.php?action=forget">'.$lang_login['Forgotten pass'].'</a>');
//echo $cur_user['group_id']."<br>";
