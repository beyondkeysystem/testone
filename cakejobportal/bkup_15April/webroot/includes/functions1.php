...
<br><br>";		
								 	...
								 	//echo $msg; exit;
									//echo $rsSeeker->seeker_email; exit;
								// $headers = "From: $from\nCc:deepa.budhani@alpha-analytics.com";
								 $headers = "From:$from\nContent-type: text/html; charset=iso-8859-1\r\n";
								 //deepa.budhani@alpha-analytics.com
								 //$sendmail = mail("rakesh.joshi@alpha-analytics.com", $subject, $msg, $headers);	
								 //$sendmail = mail("brendonmorris@gmail.com", $subject, $msg, $headers);	
								
								 $sendmail = mail(trim($rsSeeker->seeker_email), $subject, $msg, $headers);		
								//echo "success";exit;	
								 $data = array();
								 $data["mail_sent"] = 1;
	...
	{	
		...
		$objDb->DeleteData("job_msgbox","ad_id",$ad_id);
		$objDb->DeleteData("job_rec_alert_dashboard","ad_id",$ad_id);
		...
		$seeker_citizenship = $rsJob->filter_citizenship !="" ? $rsJob->filter_citizenship : "" ;
		$seeker_filter_industry = $rsJob->filter_industry_filter !="" ? $rsJob->filter_industry_filter : "" ;
		$seeker_industry = $rsJob->industry !="" ? $rsJob->industry : "" ;
				...
				$alert_mail_seeker_id = $rsSeeker1->seeker_id ;
					...
					//for job alert
						...
						if($rsJob->vacansy!='Bursary application Position' )
						{
							...
							if($seeker_citizenship != "" and $rsSeeker1->seeker_citizenship != "") {
								$sqlSeeker .= " and (seeker_citizenship='" . $seeker_citizenship . "') " ;
								$alert_search_flag = true ;
							}
			
							$sqlSeeker .= " and (seeker_email_alert=1) " ;
							$alert_search_flag = true ;
...
<br>";
								...
								else if($rsJob->vacansy!='Bursary application Position')
								{
								    ...
								    //echo "<br>$sqlSeeker<br>";
									//exit; 
									
									 
									 //$objDb->InsertData("job_recruiter_temp",$arrayJobAlert);
...
<br><br>";		
									 ...
									 $arrayJobAlertMsg['receiver_id'] = "J-".$rsSeeker->seeker_id;
									 $arrayJobAlertMsg['sender_id'] = "R-".$_SESSION['ses_rec_id'];
									 $arrayJobAlertMsg['subject'] = $subject;	
									 $arrayJobAlertMsg['body'] = $msg;	
									 $arrayJobAlertMsg['msg_date'] = date("Y-m-d");	
									 $arrayJobAlertMsg['ad_id'] = $ad_id;
									
									 $objDb->InsertData("job_msgbox",$arrayJobAlertMsg);	
									
									 $arrayJobAlertDashboard['seeker_id'] = $rsSeeker->seeker_id;
									 $arrayJobAlertDashboard['rec_id'] = $_SESSION['ses_rec_id'];
									 $arrayJobAlertDashboard['ad_id'] =  $ad_id;
								     $arrayJobAlertDashboard['citizenship'] =  $rsJob->filter_citizenship;
									 $arrayJobAlertDashboard['keywords'] = $rsJob->seeker_work_cat.", ".$rsJob->seeker_work_subcat;
									
									 $objDb->InsertData("job_rec_alert_dashboard",$arrayJobAlertDashboard);			
						 		 }
								 	//echo $msg; exit;
									//echo $rsSeeker->seeker_email; exit;
								// $headers = "From: $from\nCc:deepa.budhani@alpha-analytics.com";
	...
	function get_registration_type() {
	   ...
	   $res_chk =  $objDb->ExecuteQuery($sql_chk);
	...
	function fill_dropdown($name, $table, $textField, $valueField, $selectedValue = "", $firstValue = "Select", $where="", $attributes="",$orderOption="",$id="")
	{
        ...
        if($id!="")
		{
			...
			if($where=="where category_id=15")
                        $str = "<select onchange='showOption(this.value,this.id)' name='" . $name . "'" ."   ". $attributes . ">";
			else
	...
	{
	 	if ($status == 1) 
			echo "<span id='status_" . $id . "'><a href='#' class='paging_text' title='In-activate " . $alertname . "?' onclick=\"setStatus('" . $alertname . "','" . $id_head . "'," . $id . ",'" . $status_head . "',0,'" . $table . "');\">Active</span>"; 
		else 
			echo "<span id='status_" . $id . "'><a href='#' class='paging_text' title='activate " . $alertname . "?' onclick=\"setStatus('" . $alertname . "','" . $id_head . "'," . $id . ",'" . $status_head . "',1,'" . $table . "');\">Inactive</span>"; 
	}
...
function aplliedJobseejersCount($rec_id,$ad_id)
{
	
	global $objDb ;
	$shortlisted = 0;
	$sqlApplied = "select count(*) as cnt from job_appliedjobs aj, job_jobseeker js, job_ad j,job_rec_payment_plans p where p.plan_id=j.plan_id and p.rate>0  and js.seeker_id = aj.seeker_id and j.ad_id=aj.ad_id and aj.applied_status not in(1) and j.job_status=1 and j.vacansy <> 'Bursary application Position' and j.rec_id=" . $rec_id ." and  j.ad_id=" . $ad_id ." order by aj.total_score desc";
					   
	$resultApplied = $objDb->ExecuteQuery($sqlApplied);
	
	$rsApplied = mysql_fetch_object($resultApplied);
	  
	return $rsApplied->cnt;
}
function aplliedAllJobseejersCount($rec_id)
{
	
	global $objDb ;
	$shortlisted = 0;
	$sqlApplied = "select count(*) as cnt from job_appliedjobs aj, job_jobseeker js, job_ad j,job_rec_payment_plans p where p.plan_id=j.plan_id and p.rate>0  and js.seeker_id = aj.seeker_id and j.ad_id=aj.ad_id and aj.applied_status not in(1) and j.job_status=1 and j.vacansy <> 'Bursary application Position' and j.rec_id=" . $rec_id ." order by aj.total_score desc";
...
function shortListedCount($rec_id)
{
	
	global $objDb ;
	$shortlisted = 0;
	$sqlShortlisted = "SELECT count(*) as cnt FROM job_shortlisted_jobseekers j, job_rec_invoices i, job_rec_payments p 
					   WHERE p.invoice_id=i.invoice_id and i.invoice_id=j.invoice_id 
					   AND i.rec_id=" . $rec_id . " 
					   AND j.status=1";
	...
	$rsShortlisted = mysql_fetch_object($resultShortlisted);
	  
	return $rsShortlisted->cnt;
}	
function talentListedCount($rec_id)
{
	
	global $objDb ;
	$shortlisted = 0;
	$sqlShortlisted = "select distinct(aj.seeker_id) from job_appliedjobs aj, job_ad j, job_recruiter jr where jr.rec_id = j.rec_id and ( j.ad_id = aj.ad_id or aj.recruiter_id = $rec_id ) and ( aj.talent=1) and j.job_status=1 and jr.rec_id=" . $rec_id;
					   
	$resultShortlisted = $objDb->ExecuteQuery($sqlShortlisted);
	
	$rsShortlisted = mysql_num_rows($resultShortlisted);
	  
	return $rsShortlisted;
}
...
function createDateMonthYear($name,$value="",$array="")
	...
	// This is newly added function to search the seeker already applied for this job
	function searchAppliedJobSeekers($seekId){ 
	
		global $objDb ;
		$objDb->Connection() ;
		$sqlAJS1 = "select * from job_appliedjobs where seeker_id=" . $seekId . " and ad_id=0 and recruiter_id = ".$_SESSION["ses_rec_id"]. ""; 
		$sqlResult1 = $objDb->ExecuteQuery($sqlAJS1);		
		$num1 = mysql_num_rows( $sqlResult1 ); 
		
		$sqlAJS2 = "SELECT aj.* FROM job_appliedjobs as aj, job_ad as j , job_recruiter as jr WHERE jr.rec_id = j.rec_id and j.ad_id = aj.ad_id and jr.rec_id = ".$_SESSION["ses_rec_id"]. " and seeker_id = " . $seekId ; 
		$sqlResult2 = $objDb->ExecuteQuery($sqlAJS2);		
	...
	// This is newly added function to search the seeker already shortlisted
	function searchShortlistedJobSeekers($seekId){ 
	
		global $objDb ;
		$objDb->Connection() ;
		$sqlShortlisted = "SELECT * FROM job_shortlisted_jobseekers j, job_rec_invoices i, job_rec_payments p 
													   WHERE p.invoice_id=i.invoice_id and i.invoice_id=j.invoice_id 
													   AND i.rec_id=" . $_SESSION["ses_rec_id"] . " 
													   AND j.seeker_id=" . $seekId . " 
													   AND j.status=1";
		$sqlResult = $objDb->ExecuteQuery($sqlShortlisted);
		//$sqlResult = $objDb->ExecuteQuery($sqlAJS);		
...
// this is newly added on 15 Apr 2010
	function searchShortlisted( $sid ) {
		global $objDb ;
		$objDb->Connection() ;
		$sqlShortlisted = " SELECT * FROM job_shortlisted_jobseekers WHERE shortlist_id = ". $sid;
		$sqlResult = $objDb->ExecuteQuery($sqlShortlisted);
		//$sqlResult = $objDb->ExecuteQuery($sqlAJS);		
		$num = mysql_num_rows( $sqlResult ); 
		if($num > 0 ){
	...
	// newly added function for showing position
function showPosition( $pid ) {
	$objDb = new db();
	$sqlJobs = "select position_name from job_ad where ad_id=" . $pid ;
	$resultJobs = $objDb->ExecuteQuery($sqlJobs);
	if( mysql_num_rows($resultJobs) > 0 ){
		$posname = mysql_fetch_object( $resultJobs );
		return $posname->position_name;
	}
	else{
		return "";
	}
...
function getAppliedSeeker($ad_id){ 
	
		global $objDb ;
		$objDb->Connection() ;
		$sqlAJS1 = "select * from job_appliedjobs where applied_id =" . $ad_id ; 
		$sqlResult1 = $objDb->ExecuteQuery($sqlAJS1);		
		$num1 = mysql_num_rows( $sqlResult1 ); 		
		if($num1 > 0 ){
				return $sqlResult1 ;
...
function create_lang_var_name($str="")
{
...
function add_to_favourite($info)
{	
	$query="INSERT INTO `job_ad_bookmark` (`bookmark_id`, `rec_id`, `ad_id`, `ad_type`) VALUES (NULL, '".$info['rec_id']."', '".$info['ad_id']."', '".$info['ad_type']."');";
	mysql_query($query);
	//print_r($_REQUEST);
}

function fetchDetail($field,$table,$where)
	{
		global $db;
...
function sendMailseeker($seekerid,$recid) 
	{	
		$objDb = new db();	
		...
		<table width="100%" cellpadding="6" cellspacing="0" class="normal">
			...
			<tr>
				<td class="sectionheading" colspan="3">&nbsp;&nbsp;'.$rsComp->rec_email.' Company has shown interest in your Resume</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="3" class="sectionheading">&nbsp;&nbsp;</td>
		 		 ...
		 		 $str .= '</table>';	
				 $from = $adminemail;
				 $subject = "" . $rsJob->seeker_name. "a new company view your resume";	
				 $headers = "From: $from\nContent-Type: text/html; charset=iso-8859-1";
