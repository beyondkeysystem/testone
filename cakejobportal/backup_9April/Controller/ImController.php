...
class ImController extends AppController {
    ...
    var $uses = array('ResetPassword', 'TransFilmmakersChat', 'TransMember', 'TransMemberMembershipScheme', 'TransMemberVideo', 'TransVideoCc');
		...
		for ($i = 0; $i <= count($user_info); $i++) {
			...
			if($user_data['User']['status']=='active'){
			    $str .= '<li>
				    ...
				    <a href="'.$this->webroot.'im/my_chat_room?chat_with=' . base64_encode($r->trans_filmmakers_chat->from_id) . '&id=' . $_REQUEST['video_id'] . '" target="_blank" class="inst_msg_txt">' . $user_info_obj->first_name . ' ' . $user_info_obj->last_name . '<span class="status"></span></a>
			    ...
			    $str .= '<li>
				<a href="'.$this->webroot.'im/my_chat_room?chat_with=' . base64_encode($r->trans_filmmakers_chat->from_id) . '&id=' . $_REQUEST['video_id'] . '" target="_blank" class="inst_msg_img"><img src="' . $profile_image . '" alt="" /></a>
				<a href="'.$this->webroot.'im/my_chat_room?chat_with=' . base64_encode($r->trans_filmmakers_chat->from_id) . '&id=' . $_REQUEST['video_id'] . '" target="_blank" class="inst_msg_txt">' . $user_info_obj->first_name . ' ' . $user_info_obj->last_name . '<span class="off"></span></a>
				<div class="clearfix"></div>
			    </li>';
			}
