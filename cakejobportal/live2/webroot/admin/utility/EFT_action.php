<? session_start();
	...
	for($i=0;$i<count($_POST["chk"]);$i++)
	{
		...
		if($_POST[$status]!="")
		{
			...
			if(isset($_GET["link"]) && $_GET["link"]=="rec")
			{
				...
				$msg.="Your EFT amount is received successfully.<br><br>";
				
				//for cheque
				$msg.="Your EFT details are shown below :<br><br>";
				$msg.="
					<table width='95%' cellpadding='6' cellspacing='0' class='table_black_border'>
					  <tr>
						<td class='table_head'>Bank Account Number</td>
						
						<td class='table_head'>Bank Name</td>
						<td class='table_head'>Branch</td>
						<td class='table_head_end'>Payment Amount</td>
					  ...
					  $msg.="
					  <tr>
						<td width='11%' class=".$td_class.">".$rsCheque->account_number."</td>
	...
	if(isset($_GET["link"]) && $_GET["link"]=="rec")
	{
		header("location:EFT_list.php?msg=edit");
	}
?>