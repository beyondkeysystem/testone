...
class Index extends CI_Controller
{
	...
	function create()
	{
		...
		$this -> form_validation -> set_rules('subject', 'Subject', 'trim|required|max_length[30]|xss_clean');
		$this -> form_validation -> set_rules('body', 'Query', 'trim|required|xss_clean');
		$this -> form_validation -> set_rules('department', 'Department', 'required|xss_clean');
		$this -> form_validation -> set_rules('priority', 'Priority', 'required|alpha');
		...
		else
		{
			...
			if ($this -> db -> affected_rows())
			{
				...
				$ticket_link = site_url() . 'tickets/viewticket/' . $clienthash . '/' . $ticketid .'/' .$user_id;
