...
class Admin_property extends CI_Controller {
    ...
    */
    ...
    public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            $this->form_validation->set_rules('min_property_share_limit', 'Current min. Limit', 'required|numeric|callback_float_value_check');
            $this->form_validation->set_rules('min_owned_limit', 'Min. Owned Limit', 'required|numeric|callback_float_value_check');
            ...
            $this->form_validation->set_rules('built_up', 'Built Up (in Sq. ft)', 'numeric');
    ...
    */
    public function update()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            $this->form_validation->set_rules('property_bedrooms', 'Property bedrooms', 'required|numeric');
            $this->form_validation->set_rules('property_bathrooms', 'Property bathrooms', 'required|numeric');
            $this->form_validation->set_rules('built_up', 'Built Up Area (in Sq. ft)', 'numeric');
            $this->form_validation->set_rules('min_property_share_limit', 'Current Minimum Share Limit', 'required|numeric|callback_float_value_check');
            $this->form_validation->set_rules('min_owned_limit', 'Minimum Owned Limit', 'required|numeric|callback_float_value_check');
            ...
            if ($this->form_validation->run())
            {
                ...
                $min_property_share_limit = str_replace('"', '', $this->input->post('min_property_share_limit'));
                $min_owned_limit = str_replace('"', '', $this->input->post('min_owned_limit'));
                $built_up = str_replace('"', '', $this->input->post('built_up'));
                $tenure = str_replace('"', '', $this->input->post('tenure'));
                $land_area = str_replace('"', '', $this->input->post('land_area'));
    ...
    */
    ...
    public function float_value_check($str){
                        
        if ($str == number_format((float)$str, 1, '.', ''))
        {
            return TRUE;
        }
        else
        {
            $this->form_validation->set_message('float_value_check', 'The %s field contains only single decimal value!');
            return FALSE;
        }
    }

    public function block_selling_property($id){
        $data['property_enable_disable'] = 2; //Blocked        
        $this->property_model->update_sell_property_status($id,$data);
        redirect('admin/property');
    }
}
