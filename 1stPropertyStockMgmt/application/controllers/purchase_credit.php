    ...
    */
    ...
    public function pay_credit(){
        ...
        //form validation
            ...
            $this->form_validation->set_rules('credit', 'Deposit Amount', 'required|numeric|xss_clean');
            // $this->form_validation->set_rules('payment_type', 'Property location', '');
            // $this->form_validation->set_rules('datetime', 'Property location', '');
            $this->form_validation->set_rules('payee_name', 'Payee Name', 'required|xss_clean');
            $this->form_validation->set_rules('address', 'Address', 'required|xss_clean');
            $this->form_validation->set_rules('location', 'Payment Location', 'required|xss_clean');
            $this->form_validation->set_rules('bank_name', 'Bank Name', 'xss_clean');
            $this->form_validation->set_rules('account_no', 'Bank Account Number', 'xss_clean|numeric');
            $this->form_validation->set_rules('cheque_no', 'Cheque Number', 'xss_clean');
            ...
            if ($this->form_validation->run()){
                ...
                redirect('dashboard/2/1?error=error');
            }    
        }
    }    
	  ...
	  public function withdraw()
	{
	    ...
	    if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run() == FALSE){
                ...
                $upd_user_amt = $upd_debit - $upd_credit;

                if($upd_user_amt >= $amount){
                ...
                redirect('dashboard/2/2?error=error');
            }
        }
	}
