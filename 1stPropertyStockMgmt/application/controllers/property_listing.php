    ...
    */	
	...
	$data['available']=array();
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST' )
            ...
            $this->form_validation->set_rules('property_name', 'Property name', 'xss_clean');
            $this->form_validation->set_rules('property_location', 'Property location', 'xss_clean');
	...
	function property_details()
	{
		...
		$data['property_description'] = $property_table_details[0]['property_description'];
		...
		if($this->session->userdata('is_logged_in')){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST')
			{
				...
				if($property_status == md5("pending") || $property_status == md5("selling")){

					/* Post value from the view */
					...
					$share_limits = $this->input->post('share_limits');
					...
					/* Value fetch from the database */
					$upd_property_details = $this->property_model->get_product_by_id($id);
					$upd_property_current_price = $upd_property_details[0]['property_current_price'];
					$upd_property_enable_disable = $upd_property_details[0]['property_enable_disable']; //1
					$upd_min_property_share_limit = $upd_property_details[0]['min_property_share_limit'];


					$upd_user_property = $this->user_property_model->get_property_id1($property_id,$user_id);
					$upd_property_share_in_per = $upd_user_property['property_share_in_per'];
					
					$upd_user_property_calc = $this->user_property_model->get_property_cal($property_id);

					$upd_property_share_in_per = $upd_user_property_calc['property_share_in_per'];

					$upd_user_credit = $this->user_credit_model->get_user__by_userid($user_id);
					$upd_credit = $upd_user_credit[0]['credit'];
					// pr($upd_user_credit);die;

					$upd_creditData =  $this->credit_price_model->get_credit_price();
					$upd_credit_price = $upd_creditData[0]['price'];

					$upd_buyer_amt = $upd_credit * $upd_credit_price;

					// pr($creditData);die;

					//echo "<pre>";print_r($upd_user_credit);die;
					
					$upd_role =  $this->transactionfees_model->get_transactionfees_by_type($role);
					$upd_transaction_precentage = $upd_role['percentage'];
					
					$upd_share_price = ($buy_percent * $upd_property_current_price)/100;
					
					$upd_transaction_price = ((($buy_percent * $upd_property_current_price)/100) * $upd_transaction_precentage) / 100; 

					$upd_total = $upd_transaction_price + $upd_share_price;

					$new_number_buy_percent = number_format((float)$buy_percent, 1, '.', '');
					
					// if($upd_property_enable_disable == 1 && number_format($upd_total) == number_format($total_share_price) && number_format($upd_share_price) == number_format($purchased_share_rate) && $upd_buyer_amt >= $upd_total && $upd_min_property_share_limit == $share_limits && $upd_min_property_share_limit <= $buy_percent && $new_number_buy_percent == $buy_percent && ($upd_property_share_in_per + $buy_percent <= 100)  ){

					// 		echo "add";

					// }
					// die;
					// echo $upd_property_enable_disable."<br/>";
					// echo $upd_total."<br/>";
					// echo $total_share_price."<br/>";
					// echo $upd_share_price."<br/>";
					// echo $purchased_share_rate."<br/>";
					// echo $upd_buyer_amt."<br/>";
					// echo $upd_min_property_share_limit."<br/>";
					// echo $share_limits."<br/>";
					// echo $upd_property_share_in_per."<br/>";
					// echo $buy_percent."<br/>";
					// echo $new_number_buy_percent."<br/>";die;
					...
					if($upd_property_enable_disable == 1 && number_format($upd_total) == number_format($total_share_price) && number_format($upd_share_price) == number_format($purchased_share_rate) && $upd_buyer_amt >= $upd_total && $upd_min_property_share_limit == $share_limits && $upd_min_property_share_limit <= $buy_percent && $new_number_buy_percent == $buy_percent && ($upd_property_share_in_per + $buy_percent <= 100) ){
						...
						$data['transaction_status'] = "Transaction update";
				...
				** Date 26.11.14
				**/
				else if($property_status == md5("owned") && $propertySaleForPhaseTwo == "propertySaleForPhaseTwo"){
					//echo $user_id;
					// echo "<pre>";print_r($this->input->post());die;
					$share_req = $this->input->post('share_req');
					
					if($this->input->post('share_req') > 0){
						...
						foreach($share_req as $user_property_id=>$share_buy){
							...
							$min_owned_limit = $this->input->post('min_owned_limit');
							

						$transactionfees_amt = ($share_price * $total_share)*($transactionfees/100);
						
						$purchased_share_rate = $share_price * $total_share;
						...
						$upd_property_enable_disable = $upd_property_details[0]['property_enable_disable']; //1
						$upd_min_property_share_limit = $upd_property_details[0]['min_property_share_limit'];
						$upd_min_owned_limit = $upd_property_details[0]['min_owned_limit'];
						// pr($upd_property_details);die;
						...
						// pr($creditData);die;
						// echo ($total_share);die;
						
						
						$upd_role =  $this->transactionfees_model->get_transactionfees_by_type($role);
						$upd_transaction_precentage = $upd_role['percentage'];
						
						$upd_share_price = ($total_share * $upd_property_current_price)/100;
						
						$upd_transaction_price = ((($total_share * $upd_property_current_price)/100) * $upd_transaction_precentage) / 100; 

						$upd_total = $upd_transaction_price + $upd_share_price;
						
						$validate_number_decimal = false;
						
						foreach($share_req as $user_property_id=>$share_buy){
							$new_number_buy_percent = number_format((float)$share_buy, 1, '.', '');
							if($new_number_buy_percent == $share_buy){
								$validate_number_decimal = true;
							}else{
								$validate_number_decimal = false;
								break ;
							}
					    ...
					    $upd_user_property = $this->user_property_model->get_property_id_shares($id);
					    
					    $upd_sell_property_share = 0;
						$upd_property_share_in_per = 0;
					    
					    foreach($upd_user_property as $user_property){
					    	$upd_sell_property_share += $user_property['sell_property_share'];
					    	$upd_property_share_in_per += $user_property['property_share_in_per'];
					    }

					    // echo $upd_min_owned_limit."<br/>";
					    // echo $min_owned_limit;
					    // pr($upd_user_property);
					    // die;
					 ...
					 // 	echo $upd_property_enable_disable."<br/>";
						...
						// echo $total_share."<br/>";
						// echo $upd_property_share_in_per."<br/>";
						// echo "sell_property_share ".$upd_sell_property_share."<br/>";
						// echo "upd_min_owned_limit".$upd_min_owned_limit."<br/>";
						// echo "validate n ".$validate_number_decimal."<br/>";
						// echo "min_owned_limit asd  ".$min_owned_limit."<br/>";
	// if(($upd_property_enable_disable == 1 || $upd_property_enable_disable == 2) && number_format($upd_total) == number_format($total_token) && number_format($upd_share_price) == number_format($purchased_share_rate) && $upd_buyer_amt >= $upd_total && $validate_number_decimal == true && $upd_min_property_share_limit <= $total_share && ($upd_property_share_in_per+$total_share <= 100) && $upd_sell_property_share >= $total_share && $upd_min_owned_limit == $min_owned_limit ){
	// echo "asdjkhsalk";
	// }
				...
				elseif($property_status == md5("owned")){
...
/* End */
	...
	function sell_property(){
		...
		$sell_share = $_POST['sell_share'];	// Share from user to sell from input box (popup)
		$share_avail = $_POST['share_avail']; //Max available share to sell.
		// exit('as');

		if($sell_share > 0){

			if(number_format($sell_share,1) == $sell_share){
				$validate = true;
			}else{
				$validate = false;	
			}
			...
			$new_number_sell_share = number_format((float)$sell_share, 1, '.', '');
			$upd_property_share_in_per = $rowProperty['property_share_in_per'];
			$upd_sell_property_share = $rowProperty['sell_property_share'];

			// echo $rowProperty['property_share_in_per']."<br/>";
			// echo $sell_share."<br/>";
			// echo $new_number_sell_share."<br/>";
			// echo $upd_property_share_in_per."<br/>";
			// echo $upd_sell_property_share."<br/>";
			// die;
			
			if($rowProperty['property_share_in_per'] >= $sell_share && $new_number_sell_share == $sell_share && $upd_property_share_in_per<=100 && $sell_share <= $upd_property_share_in_per && $validate == true){
