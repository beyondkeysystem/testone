      ...
      <div class="item samebox minislider">
       ...
       ?>
        ...
        <ul class="icons">
          ...
          <li> <b> <?=$this->lang->line('share_available')?>: <span><?php if($available[$j] !=0){ echo number_format($available[$j],2);}else{ if($row['property_status']=='owned'){echo number_format($sold[$j],2);}else{echo 0;}}?> % </span> </b></li>
        ...
        ?>
        <div class="rm_btn"> <span><?=$arrow?></i> MYR <?=number_format($row['property_current_price'])?></span> <a href="<?php echo base_url(); ?>property_details/<?php echo $row['id'];?>"><?=$this->lang->line('read_more')?></a> </div>
      </div>
      <?php $j++; } 
    }else{
