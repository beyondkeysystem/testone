      ...
      <div class="row">
        ...
        //Code By Me Start
              ...
              foreach($withdraw as $row)
              {
                ...
                if(isset($row['status']) and $row['status'] =='Pending' and $row['balance'] >= $row['fund_amt']){
                    echo '<td class=""><input type="hidden"  name="user_id" id="user_id_'.$row['id'].'" value="'.$row['user_id'].'">
                      <input type="checkbox"  onclick="fnopenpopup(this)" class="openpopup" name="ids[]" id="check_'.$row['id'].'" value="'.$row['id'].'"></td>';
                }
                else
                    echo '<td class=""><input type="checkbox" disabled></td>';
                echo '</tr>';
            ...
            function fnopenpopup(obj){
              // alert(obj.value);
                if(document.getElementById(obj.id).checked == true){
                        var user_id = $("#user_id_"+obj.value).val();
                        // alert(user_id);
                        $.fancybox({ 
                                'width'			: '50%',
                                'height'                    : '75%', 
                                ...
                                'speedIn':350, 
                                'speedOut':350, 
                                'href':'/transaction/payment_approve/'+obj.value+'/'+user_id,
                                'type'			: 'iframe'
                        });
                    }
                }
