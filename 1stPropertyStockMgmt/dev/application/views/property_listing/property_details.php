 ...
 <div class="property_detail">
    ...
    <div class="pdetail_text">
      ...
      <div class="price_plan_sec">
        ...
        ?>
        <div class="start_price"><?=$this->lang->line('price_starts_from')?>: <strong><?=$arrow?><?php echo ($row['property_current_price'])?$row['property_current_price']:$row['property_initial_price']; ?></strong></div>
...
<?php if($check_user_property > 0){ ?>
  ...
  <?php } ?>
    ...
    ** Date: 22/12/2014
    */
    }elseif($this->session->userdata('is_logged_in') && ($tot_share_available != 100 || $sell_property_share > 1) && $row['property_enable_disable'] != 2){ ?>
    ...
    ?>
      ...
      <div class="descp_table_title">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="descp_table">
          <tr>
            ...
            <td><?=$this->lang->line('from')?> <span class="colorblue"><?php echo $row['property_initial_price']?></span></td>
          </tr>
          <?php
      ...
      <?php if($this->session->userdata('is_logged_in') ){ ?>
            ...
            <td><?php if(!empty($available)){echo number_format($available,2); }else{ echo 0; }?> %</td>
      <?php
      } 
       ?>
          </tr>
  ...
  <div class="mrkt_overview content">
    <div class="mrkt_content">
      ...
      <div class="mrkt_graph_detail">
        <p><strong>MYR <?php echo ($row['property_current_price'])?$row['property_current_price']:$row['property_initial_price']; ?></strong> <span>(Last 5 changes)</span></p>
        <ul>
...
?>
          <li><strong><?php echo $chart_date[$i]; ?></strong><span><?php $share=number_format(($chart_price[$i]-$property_detail[0]['property_initial_price'])/100, 2); if($share>0){?><i class="fa fa-arrow-up greencolor"></i><?php }else{ ?><i class="fa fa-arrow-down redcolor"></i><?php } echo $share; ?>%</span><span>MYR <?php echo number_format($chart_price[$i], 2); ?></span></li>
...
<!-- <tr>
   ...
   <td><span class="col-g"><?=$balance?></span></td>
      ...
      <div id="tab-3" class="clsdiv">
        ...
        <?php if($property_information['property_description'] != ""){
          echo $property_information['property_description'];
        }else{
...
<!--SellPopup-->
  <div id="mySellModal" class="reveal-modal popup" style="opacity: 1; top: 10%;">
    ...
    <div class="clear"></div>
  <?=$this->lang->line('sell_share_property_into')?> <span id="sell_available"><?=$property_information['property_share_in_per']?></span> <?=$this->lang->line('percentage')?>.
  <form accept-charset="utf-8" method="post" action="#">
    <div class="reg-form-row">
    <div class="row-col1">
      <label><?=$this->lang->line('sell_share_in_pre')?></label>
      
      <input type="number" step="0.1" max="<?=$property_information['property_share_in_per']?>" name="sell_share_percentage" id="sell_share_percent" placeholder="Numeric/Decimal Values (0.1 or 1...)">
   ...
   <div class="checkplan_btn"> <a id="SellSubmit" href="javascript:void(0);"><?=$this->lang->line('agree')?></a> </div>
...
<!--BuyPopup--> <!-- Mayank Pawar 22/12/2014 -->
  ...
  <?php $buyshare = 100 - $property_share_in_per; ?>
    ...
    <div class="clear"></div>
      ...
      // }else{
      ?>
        <?=$this->lang->line('you_can_buy')?> <span id="minimum_share"><?=$min_property_share_limit;?></span> to <span id="available"> <?php echo number_format($buyshare,2)?></span> <?=$this->lang->line('per_shares')?>. <!-- Mayank Pawar -->
      <?php
      // }
      ?>
