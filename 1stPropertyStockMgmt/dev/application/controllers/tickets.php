...
class Tickets extends CI_Controller
{
	...
	function viewticket($ticketid = '')
	{
		...
		if ($this -> uri -> segment('4') != '')
		{
			...
			$user=$this->Tickets->get_user_id($ticketid);
	                $ticket_user=$user[0]['user_id'];
		}
		if ($this -> uri -> segment('5') != '')
		{
			...
			if($ticket_user!=$userid){
				$this -> data['error'] = 'You are not authorised user to view this tickets';
	...
	function reply()
	{
		...
		else
		{
			...
			$this -> data['success'] = "Ticket reply has been added";
                        $clienthash = $this -> session -> userdata('user_id');
			// Send mail to staffs
