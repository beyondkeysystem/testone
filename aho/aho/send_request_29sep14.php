...
if(isset($_POST['send_request']) && $_POST['send_request']=='Submit'){
    ...
    $sql = "insert into AHO_Consumers (Phone_No,Full_Name,Email_ID,Comment,Property_Description,Receive_Option,Request_Type) VALUES('$Phone_No','$Full_Name','$Email_ID','$Comment','$Property_Description','$Receive_Option','$Request_Type')";
    ...
    if($query){
        ...
        <body>
          <p>Hello '.$Full_Name.'</p>
          <p>You have requested for the "'.$request_type.'" on <a href="http://www.americanhomesonline.com">American Homes Online</a> on '.date('d-M-Y h:i:s').'. </p>
        ...
        // To send HTML mail, the Content-type header must be set
         $headers = "From: support@americanhomesonline.com\r\n"; 
       ...
       // $Phone_No = '+919755832755';
        /*if(strlen($Phone_No)>0){
             // code to send sms starts
                ...
                'Body' => "
                    Hello ".$Full_Name."
                    You have requested for the '".$request_type."' on 'American homes Online' on ".date('d-M-Y')."
...
if(isset($_REQUEST['Email_Verification']) && $_REQUEST['Email_Verification']==1)
{
    ...
    if(!empty($consumer_id)){
        ...
        if($query && mysql_num_rows($query)>0){
            ...
            /*if(strlen($Phone_No)>0){
                 // code to send sms starts
                    ...
                    'Body' => "
                        Hello ".$Full_Name."
                        You have successfully registered for the '".$request_type."' on ".date('d-M-Y')."
         ...
         // code to find latitude and longitude of property start
            ...
            $sql = "SELECT *,
				SQRT(POW((69.1 * (GEO_Lat - $lat)) , 2 ) +
				POW((53 * (GEO_Long - $long)), 2)) AS distance
				FROM AHO_User WHERE B_ID =$APPid AND Type=3 $Middle
				ORDER BY distance ASC Limit 7";
	    $query_result = mysql_query($sql);
                ...
                for($i=0;$i<count($Agents);$i++){
                  ...
                  // $to = "sameer@mailinator.com";
                   ...
                   <body>
                     ...
                     <p>You have received a Lead for "'.$request_type.'" on <a href="http://www.americanhomesonline.com">American Homes Online</a> on '.date('d-M-Y h:i:s').'. </p>
                     ...
                     <table>
                        ...
                        '
                            ...
                            <tr>
                                <th>Time Frame</th><td>'.$Time_Frame.'</td>
                            </tr>
                            ';
                        }   
                        $message.='
			...
			'Body' => "
                            Hello ".$Agent_Name."
                            You have received a Lead for '".$request_type."' on 'American Homes Online'  on ".date('d-M-Y')."
