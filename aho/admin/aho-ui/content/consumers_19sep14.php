...
if($sql_result && mysql_num_rows($sql_result))
{
    ?><br><br>
	    <table>
		<tr>
		    <th>S.No.</th><th>Name</th><th>Phone No.</th><th>Email ID</th><th>Comment</th><th>Reg. Date</th><th>Action</th>
		</tr>
		<?php
		    $i=0;
			...
			?>
			    <tr>
				<td><?php echo ++$i;?></td><td><?php echo $row->Full_Name;?></td><td><?php echo $row->Phone_No;?></td><td><?php echo $row->Email_ID;?></td><td><?php echo substr($row->Comment,0,10).'...';?></td><td><?php echo date('d-M-Y H:i:s',strtotime($row->Registration_Date));?></td><td><a href="index.php?Page=Consumers&Update=1&ID=<?php echo $row->ID;?>">Update</a> | <a href="index.php?Page=Consumers&Remove=1&ID=<?php echo $row->ID;?>" onclick="return confirm('Do you really want to delete this record?');">Delete</a></td>
			    </tr>
			<?php
		    }
		?>
