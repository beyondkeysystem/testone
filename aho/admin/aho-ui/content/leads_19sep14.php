...
$sql = "SELECT * FROM AHO_Consumers where Verification_Status=1";
...
if($sql_result && mysql_num_rows($sql_result))
{
    ?><br><br>
	    <table>
		<tr>
		    <th>S.No.</th><th>Lead ID</th><th>Consumer Name</th><th>Email ID</th><th>Property ID</th><th>Agent ID</th><th>Reg. Date</th><th>Action</th>
		</tr>
		<?php
		    $i=0;
			...
			?>
			    <tr>
				<td><?php echo ++$i;?></td><td><?php echo $row->ID;?></td><td><?php echo $row->Full_Name;?></td><td><?php echo $row->Email_ID;?></td><td><?php if($row->Property_ID){echo $row->Property_ID;}else{echo "Not Available";}?></td><td><?php if($row->Agent_ID){echo $row->Agent_ID;}else{echo 'Lead not accepted';}?></td><td><?php echo date('d-M-Y H:i:s',strtotime($row->Registration_Date));?></td><td><!--<a href="index.php?Page=Leads&Update=1&ID=<?php echo $row->ID;?>">Update</a> |--> <a href="index.php?Page=Leads&Remove=1&ID=<?php echo $row->ID;?>" onclick="return confirm('Do you really want to delete this record?');">Delete</a></td>
			    </tr>
			<?php
		    }
		?>
