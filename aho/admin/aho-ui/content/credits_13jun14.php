...
<span style='font-size:32px; font-weight:bold; color:<?php echo ($total < 15) ? '#f00' : '#036'; ?>'>Credits Remaining: <?php echo $total; ?></span><br>
	...
	<?php if($row2['amount_status'] != 1) { ?>
		<?php 
			...
			{
				$credits=$row2['no_of_credits'];
			}
			echo $credits." Credit = $".$row2['deduct_amount']."<br>";
		?>
  ...
  <?php }else{ ?>
  <br>
  <form action="http://americanhomesonline.com/admin/index.php?Page=credits&ID=<?php echo $AID;?>" method="post">
		<input name="quantity" id="qty" placeholder="Enter Quantity" type="text" /><br />
