...
if($Update != ''){
	...
	print "<h4><font color='green'>Credit Updated</font></h4>";
        $Type = $_POST["type"];
...
$sql_result = mysql_query($sql);
   print "<div><br><p>Credit is the amount charged from the Broker and Agent.</p></div>";
   print  "<table class = 'sTable'>";
         //   echo "<tr>
      echo "<th text-align = 'center' style='font-weight: bolder; font-size: 22px; color: black;'>Type</th>";
      echo "<th style='font-weight: bolder; font-size: 22px; color: black;'>Amount (in Dollars) per Credit</th>";
           /* <th style='font-weight: bolder; font-size: 22px; color: black;'>Time Period</th>
            <th style='font-weight: bolder; font-size: 22px; color: black;'>Time Duration</th>    commented on 3 June 14
            <th style='font-weight: bolder; font-size: 22px; color: black;'>Saving Amount</th>*/
     //  echo "<th style='font-weight: bolder; font-size: 22px; color: black;'>No of Credits needed for Lead</th>";
     //   echo "<th style='font-weight: bolder; font-size: 22px; color: black;'>Amount Status</th>"; //added on 3 June 14
        //    <th style='font-weight: bolder; font-size: 22px; color: black;'>No of Leads</th>
       echo "<th style='font-weight: bolder; font-size: 22px; color: black;width:14%'>Actions</td></th>";
while ($row = mysql_fetch_array($sql_result)) { 

    if($row['type']=='credit'){ if($row['deduct_amount']<=0){$price='Free';}else{$price=$row['deduct_amount'];}
    echo "<tr style='text-align:center'>";
            echo "<td style=' font-size: 22px; color: black;'>".$row['type']."</td>
            <td style='font-size: color: black;'>".$price."</td>";
           /* <td style='font-size: 22px; color: black;'>".$row['time_period']."</td>
            <td style='font-size: 22px; color: black;'>".$row['time_duration']."</td>   commented on 3 June 14
            <td style='font-size: 22px; color: black;'>".$row['saving_amount']."</td>*/
       //   echo"  <td style='font-size: 22px; color: black;'>".$row['no_of_credits']."</td>";
	//  echo "<td style='font-size: 22px; color: black;'>".$amount_status."</td>"; //added on 3 June 14
...
<form action="index.php?Page=Subscription" method="post" data-ajax="false" data-mini="true" enctype="multipart/form-data" class="adminforms">
	<div data-role="fieldcontain" class="ui-hide-label">
		...
		<label for="adeduct_amount">Amount (in Dollars) per Credit:</label>
	...
	<div data-role="fieldcontain" class="ui-hide-label amount_status" style="<?php echo $hide;?>" >
		<label for="amount_status"><input type="checkbox" name="amount_status" id="amount_status" value="1" <?php if($row["amount_status"]==1){echo 'checked';} ?> />Allow free Credits for Brokers and Agents</label>
